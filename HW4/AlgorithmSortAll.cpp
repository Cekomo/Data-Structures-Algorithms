...
int AlgorithmSortAll::select() {
    ...
    for (int i = 1; i < n; i++) {
        ...
        while (j > 0 && key > pNums[j-1]) { // This part may cause a problem (pNums[j] > keys)
            pNums[j] = pNums[j-1]; 
            j = j - 1;
        }
        pNums[j] = key; 
    ...
    // for (int i = 0; 10*i < n; i++) // i added that piece of code to see the value for every 10th product
    //    cout << (i + 1)*10 << "'th member: " << pNums[10*i] << endl;

    int biggest_kth = pNums[(this->k) - 1]; 
